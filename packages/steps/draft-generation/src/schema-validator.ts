...
export type ListingDraftArtifactValidation =
  ...
  | {
      ok: false;
      code: 'LEGACY_DRAFT_CONTRACT_UNSUPPORTED' | 'DRAFT_SCHEMA_INVALID';
      errors: string[];
    };
...
export function validateListingDraftArtifact(value: unknown): ListingDraftArtifactValidation {
  if (isRecord(value) && value.schema_version === 1) {
    return {
      ok: false,
      code: 'LEGACY_DRAFT_CONTRACT_UNSUPPORTED',
      errors: ['ListingDraftV1 is read-only and cannot be published.'],
    };
  }
