...
export function validateCostPricingSchema(value: unknown): { valid: boolean; errors: ErrorObject[] } {
...
function resolveSchemaPath(): string {
  ...
  const candidates = [
    path.join(repoRoot, 'packages/steps/cost-pricing/output.schema.json'),
    path.resolve(moduleDirectory, '../output.schema.json'),
  ];
  const found = candidates.find((candidate) => fs.existsSync(candidate));
  if (!found) throw new Error(`CostPricingV1 schema not found: ${candidates.join(', ')}`);
  return found;
}
