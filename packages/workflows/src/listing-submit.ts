import type { CommandResult, ListingDraftV1, OzonPublishResultV1, StorePublishProfileV1 } from '@auto-ozon/contracts';
...
async function runListingPublishLocked(options: ListingSubmitOptions, store: ArtifactStore): Promise<CommandResult<OzonPublishResultV1>> {
  ...
  let apiKey: string;
  try {
    const storeProfile = new FileStoreRegistry().get(options.store_id);
  ...
  return runListingSubmit({ draft, profile, transport: new OzonSellerImportClient({ clientId, apiKey }), previous: previous ?? undefined }, context);
}
